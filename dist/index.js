...
`,c=async n=>new Promise(t=>{const r=setInterval(()=>{const a=document.querySelector(n);a&&(clearInterval(r),t(a))},100)}),d=document.createElement("style");d.innerHTML=u;document.head.appendChild(d);const i="lock-merge",h=()=>{const n=document.createElement("input");n.type="checkbox",n.id="lock-merge",document.body.appendChild(n),localStorage.getItem(i)==="checked"&&(n.checked=!0),n.addEventListener("change",()=>{localStorage.setItem(i,n.checked?"checked":"unchecked")})},g=()=>{const n=document.querySelector(".partial-pull-merging-analytics-js section + div");if(!n)return;const t=document.createElement("a");t.href="https://github.com/simplicate-software/frontend/actions?query=actor:joosthobmasimplicate",t.innerText="Actions of this PR",n.append(t)},p=async()=>{console.log("github loaded"),h(),m()};c(".timeline-comment-group").then(p);const f=async()=>{console.log("pipeline loaded"),g()};c("[class*='MergeBoxSectionHeader-module__wrapper']").then(f);
