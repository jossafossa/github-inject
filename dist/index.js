...
`,c=async n=>new Promise(t=>{const r=setInterval(()=>{const a=document.querySelector(n);a&&(clearInterval(r),t(a))},100)}),d=document.createElement("style");d.innerHTML=l;document.head.appendChild(d);const i="lock-merge",m=()=>{const n=document.createElement("input");n.type="checkbox",n.id="lock-merge",document.body.appendChild(n),localStorage.getItem(i)==="checked"&&(n.checked=!0),n.addEventListener("change",()=>{localStorage.setItem(i,n.checked?"checked":"unchecked")})},u=()=>{const n=document.querySelector(".partial-pull-merging-analytics-js section + div");if(!n)return;const t=document.createElement("a");t.href="https://github.com/simplicate-software/frontend/actions?query=actor:joosthobmasimplicate",t.innerText="Actions of this PR",n.append(t)},h=()=>document.querySelector(".gh-header-meta [title='Status: Merged']")?"merged":document.querySelector("[reviewable_state='draft']")?"draft":document.querySelector("[data-name*='QA-Test'], [data-name*='qa-test']")?"test":!1,g=()=>{let n=!1;const t=setInterval(()=>{const r=h();if(n!==r&&r){const a=`[${r.toUpperCase()}]`;n=!!r;const e=document.querySelector("h1 > bdi");e&&(e.innerHTML=`${a} ${e.innerHTML}`),document.title=`${a} ${document.title}`,clearInterval(t)}},1e3)},p=async()=>{console.log("github loaded"),m(),g()};c(".timeline-comment-group").then(p);const f=async()=>{console.log("pipeline loaded"),u()};c("[class*='MergeBoxSectionHeader-module__wrapper']").then(f);
